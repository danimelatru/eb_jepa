...
Run: python -m examples.ac_video_jepa.maze.main_cotrain <fine_ckpt> <subgoal_ckpt> <out_dir>
        [N=4] [epochs=8] [freeze_epochs=3] [enc_lr=5e-5] [distill_coeff=10.0]
"""
import os
import shutil
import sys
...
def main():
    ...
    distill_coeff = float(sys.argv[8]) if len(sys.argv) > 8 else 10.0 # latent distillation strength
    ...
    start_epoch = 0
    resume_ckpt = Path(out_dir) / "latest.pth.tar"
    if resume_ckpt.exists():
        print(f"[cotrain] Found resume checkpoint: {resume_ckpt}", flush=True)
        info = load_checkpoint(resume_ckpt, jepa, optimizer=opt, scheduler=None, device=device, strict=False)
        start_epoch = info.get("epoch", -1) + 1
        if "xy_head_state_dict" in info:
            xy_head.load_state_dict(info["xy_head_state_dict"])
        resume_sg = Path(out_dir) / "subgoal.pth.tar"
        if resume_sg.exists():
            sg_state = torch.load(resume_sg, map_location=device, weights_only=False)
            subgoal.load_state_dict(sg_state["subgoal"])
    else:
    ...
    # Create the frozen reference encoder for Latent Distillation
    frozen_enc = ImpalaEncoder(width=1, stack_sizes=(16, m.henc, m.dstc), num_blocks=2,
                        dropout_rate=None, layer_norm=False, input_channels=m.dobs,
                        final_ln=True, mlp_output_dim=512,
                        input_shape=(m.dobs, data_config.img_size, data_config.img_size)).to(device)
    frozen_enc.load_state_dict(enc.state_dict())
    frozen_enc.eval()
    for p in frozen_enc.parameters():
        p.requires_grad_(False)

    print(f"[cotrain] f={f} N={N} epochs={epochs} | joint fine-tune (shared latent) with distill_coeff={distill_coeff}", flush=True)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        jl = sgl = al = dl = 0.0; nb = 0
        for x, a, loc, _, _ in loader:
            x = x.to(device, non_blocking=True).float(); a = a.to(device, non_blocking=True).float()
            loc = loc.to(device, non_blocking=True).float()
            B, _, T = loc.shape
            ...
            distill_loss = F.mse_loss(z, z_frozen)
            
            total = jepa_loss + aux_coeff * aux + sg_coeff * sg_loss + distill_coeff * distill_loss
            opt.zero_grad(); total.backward(); opt.step()
            jl += float(jepa_loss); sgl += float(sg_loss); al += float(aux); dl += float(distill_loss); nb += 1
        print(f"[cotrain] epoch {epoch} {time.time()-t0:.0f}s jepa={jl/nb:.4f} "
              f"subgoal={sgl/nb:.5f} aux={al/nb:.5f} distill={dl/nb:.5f}", flush=True)
        save_checkpoint(Path(out_dir) / f"epoch_{epoch}.pth.tar", model=jepa, epoch=epoch,
        ...
        torch.save({"subgoal": subgoal.state_dict(), "N": N, "f": f},
                   Path(out_dir) / f"subgoal_{epoch}.pth.tar")
        torch.save({"subgoal": subgoal.state_dict(), "N": N, "f": f},
